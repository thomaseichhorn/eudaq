...
public:                                                                        \
  ...
  virtual unsigned get_id() const { return eudaq_static_id(); }                \
                                                                               \
private:                                                                       \
...
namespace eudaq {
  ...
  public:
    ...
    Event(unsigned run, unsigned event, uint64_t timestamp = NOTIMESTAMP,
          unsigned flags = 0)
  ...
  public:
    static Event *Create(Deserializer &ds) {
      ...
      if (!cr)
        EUDAQ_THROW("Unrecognised Event type (" + Event::id2str(id) + ")");
      return cr(ds);
    }
   ...
   */
  template <typename T_Evt> struct RegisterEventType {
    RegisterEventType() {
      EventFactory::Register(T_Evt::eudaq_static_id(), &factory_func);
    }
    static Event *factory_func(Deserializer &ds) { return new T_Evt(ds); }
  };
}
