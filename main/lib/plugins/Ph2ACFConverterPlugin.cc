  ...
  public:
    ...
    virtual bool GetLCIOSubEvent(lcio::LCEvent & lcioEvent, const Event & eudaqEvent) const
    {
      ...
      for(size_t cSensor = 0; cSensor < ev_raw.NumBlocks(); ++cSensor)
      {
		...
		sparseFrame->emplace_back ( getlittleendian<unsigned short>(&buffer[value_offset+0]),getlittleendian<unsigned short>(&buffer[value_offset+2]),getlittleendian<unsigned short>(&buffer[value_offset+4]), 0 );
                value_offset += 6;
            }
      }
